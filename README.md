# IMU-CET Mock Test

A comprehensive mock test application for IMU-CET (Indian Maritime University Common Entrance Test) preparation.

## Features

- **Three Difficulty Levels**: Beginner, Medium, and Hard
- **200 Questions per Level**: Covering Physics, Chemistry, Mathematics, English, General Knowledge, and Aptitude
- **Real-time Timer**: 180 minutes countdown timer
- **Interactive UI**: Instant feedback on answers with color-coded options
- **Question Palette**: Easy navigation between questions
- **Progress Tracking**: Visual progress bar and detailed results
- **Responsive Design**: Works on desktop and mobile devices

## Technologies Used

- HTML5
- CSS3
- Vanilla JavaScript

## How to Use

1. Select a difficulty level (Beginner, Medium, or Hard)
2. Answer the questions by clicking on the options
3. Navigate between questions using Previous/Next buttons or the question palette
4. Submit the test when completed
5. View your detailed results

## Live Demo

[Add your Vercel deployment URL here]

## Local Development

Simply open `index.html` in your web browser to run the application locally.

## License

MIT License
